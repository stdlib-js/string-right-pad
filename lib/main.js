...
function rpad( str, len, pad ) {
	var p;
	if ( !isString( str ) ) {
		throw new TypeError( format( '1PN3F', str ) );
	}
	if ( !isNonNegativeInteger( len ) ) {
		throw new TypeError( format( '1PN3X', len ) );
	}
	if ( arguments.length > 2 ) {
		p = pad;
		if ( !isString( p ) ) {
			throw new TypeError( format( '1PNAO', p ) );
		}
		if ( p.length === 0 ) {
			throw new RangeError( format('1PN1X') );
		}
	} else {
		p = ' ';
	}
	if ( len > FLOAT64_MAX_SAFE_INTEGER ) {
		throw new RangeError( format( '1PNAP', len ) );
	}
