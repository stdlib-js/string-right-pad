...
function rpad( str, len, pad ) {
	var p;
	if ( !isString( str ) ) {
		throw new TypeError( format( '1PN3F,Ex', str ) );
	}
	if ( !isNonNegativeInteger( len ) ) {
		throw new TypeError( format( '1PN3X,F1', len ) );
	}
	if ( arguments.length > 2 ) {
		p = pad;
		if ( !isString( p ) ) {
			throw new TypeError( format( '1PNAO,M6', p ) );
		}
		if ( p.length === 0 ) {
			throw new RangeError( format('1PN1X,EF') );
		}
	} else {
		p = ' ';
	}
	if ( len > FLOAT64_MAX_SAFE_INTEGER ) {
		throw new RangeError( format( '1PNAP,M7', len ) );
	}
